...
# df_augmented = pd.read_csv('NicolaLavecchia/new_test_images.csv')
# df = pd.concat([df_originale, df_augmented])
...
for i in range(len(num_k)):
    if i == 0:
        model.add(Conv2D(num_k[i], (k_size[i], k_size[i]), activation='relu', input_shape=(28, 28, 1), kernel_regularizer=l2(0.001)))
    else:
        model.add(Conv2D(num_k[i], (k_size[i], k_size[i]), activation='relu', kernel_regularizer=l2(0.001)))
...
# Converti img_list in un array numpy
...
for i, idx in enumerate(indices):
    ...
    predicted_label = predicted_classe[idx]
